    ...
    */
    ...
    for (int k = 0; k < h_C * tZ2; k++) {
        checkCudaErrors(cufftExecC2C(plan_advY, d_result + (k * X * Y),
            d_result + (k * X * Y), CUFFT_FORWARD));
    }
    cudaDeviceSynchronize();
    //transport to host
    checkCudaErrors(cudaMemcpy(h_signal, d_result, gpu_mem_size_b,
        cudaMemcpyDeviceToHost));
    cout << "y fft xyz\n";
    for (int z = 0; z < Z; z++) {
        for (int y = 0; y < Y; y++) {
    ...
    checkCudaErrors(cufftPlanMany(&plan_advZ2, 1, n, inembed, istride, idist,
        onembed, ostride, odist, CUFFT_C2C, batch));
    checkCudaErrors(cufftExecC2C(plan_advZ2, reinterpret_cast<cufftComplex*>(d_result),
    ...
    checkCudaErrors(cufftExecC2C(plan_advX, d_result,
        d_result, CUFFT_FORWARD));

    checkCudaErrors(cudaMemcpy(h_result, d_result, gpu_mem_size_b,
        cudaMemcpyDeviceToHost));
    cout << "result fft xyz\n";
    for (int z = 0; z < Z; z++) {
        for (int y = 0; y < Y; y++) {
