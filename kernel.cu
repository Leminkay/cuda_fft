   ...
   // cout << it_s << "\n";
    ...
    for (unsigned int i = 0; i < it_s; i += 1) {
        ...
        for (unsigned int j = 0; j < C; j++) {
            if (j == C / 2)
                checkCudaErrors(cudaMemcpyAsync(d_signal, buffer, C / 2 * W * sizeof(Complex), cudaMemcpyHostToDevice));            
            memcpy(buffer + (j * W), h_signal + (i * W) + (j * X * Y * Z2), W * sizeof(Complex));
            
        }
        checkCudaErrors(cudaMemcpyAsync(d_signal + C / 2 * W , buffer + C / 2 * W, C / 2 * W * sizeof(Complex), cudaMemcpyHostToDevice));
